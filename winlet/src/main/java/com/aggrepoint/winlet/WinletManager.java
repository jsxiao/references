...
public class WinletManager implements WinletConst {
	...
	static Object getWinlet(ReqInfo req, WinletDef winletDef) {
	...
	static void putWinlet(ReqInfo req, WinletDef winletDef, Object winlet) {
		Hashtable<String, Object> ht = getWinletHashtable(winletDef.getScope(),
				req.getSession());
	...
	public static WinletInstance getWinInstance(ApplicationContext context,
		...
		Vector<WinletInstance> wis = null;
	...
	public static ViewInstance getViewInstance(ApplicationContext context,
			ReqInfo req, ViewDef def) throws Exception {
		ViewInstance vi = getWinInstance(context, req, def).findView(
				req.getViewId());
		if (vi == null) {
			// TODO: error handling
			getWinInstance(context, req, def);
		}
