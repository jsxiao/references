...
{
	private static final long serialVersionUID = 583357699880110608L;
	 ...
	 * @return
	 */
	public static MyDate setDate(Date time)
	{
		if (time != null)
		{
			return new MyDate(time.getTime());
	...
	public MyDate addMonths(int delay)
	{
		...
		c.setTime(this);
		c.add(Calendar.MONTH, delay); // 将当前日期加一个月
	...
	public MyDate addYears(int delay)
	{
		Calendar c = Calendar.getInstance();
		c.setTime(this);
		c.add(Calendar.YEAR, delay); // 将当前日期加一个年
		return new MyDate(c.getTime().getTime());
	}
	...
	{
		MyDate d2 = new MyDate(Converts.StrToDate(this.toString()));
	...
	public MyDate getLastMonthDay(MyDate mydate)
	{
		...
		case 2:
			if (getIsLeapYear())
			{
				return new MyDate(Converts.StrToDate(mydate.toString("yyyy-MM-29")));
			}
			else
			{
				return new MyDate(Converts.StrToDate(mydate.toString("yyyy-MM-28")));
			}
		case 3:
			return new MyDate(Converts.StrToDate(mydate.toString("yyyy-MM-31")));
		case 4:
...
//
//		/*
//		 * SimpleDateFormat foo = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss"); System.out.println("foo:"+foo.format(new Date()));
//		 * 
//		 * Calendar gc = GregorianCalendar.getInstance(); System.out.println("gc.getTime():"+gc.getTime()); System.out.println("gc.getTimeInMillis():"+new Date(gc.getTimeInMillis()));
//		 * 
//		 * //当前系统默认时区的时间： Calendar calendar=new GregorianCalendar(); System.out.print("时区："+calendar.getTimeZone().getID()+"  "); System.out.println("时间："+calendar.get(Calendar.HOUR_OF_DAY)+":"+calendar.get(Calendar.MINUTE)); //美国洛杉矶时区 TimeZone tz=TimeZone.getTimeZone("America/Los_Angeles"); //时区转换 calendar.setTimeZone(tz); System.out.print("时区："+calendar.getTimeZone().getID()+"  "); System.out.println("时间："+calendar.get(Calendar.HOUR_OF_DAY)+":"+calendar.get(Calendar.MINUTE)); Date time=new Date();
//		 * 
...
//		 * 
//		 * //4、从本地时间里扣除这些差量，即可以取得UTC时间： cal.add(java.util.Calendar.MILLISECOND, -(zoneOffset + dstOffset));
//		 * 
//		 * //之后调用cal.get(int x)或cal.getTimeInMillis()方法所取得的时间即是UTC标准时间。 System.out.println("UTC:"+new Date(cal.getTimeInMillis()));
//		 * 
//		 * Calendar calendar1 = Calendar.getInstance(); TimeZone tztz = TimeZone.getTimeZone("GMT"); calendar1.setTimeZone(tztz); System.out.println(calendar.getTime()); System.out.println(calendar.getTimeInMillis());
//		 */
//	}
	
	
	/**
	 * 获得当前年月日的数字形式，例如:2013-10-14 10:45:28 （n = 1）
	 * 执行后返回 2013-10-15 10:45:28
	 * @param n
	 * 					当前日期加N天
	 * @return Date
	 */
	...
	/**
	 * 获得指定年月日的数字形式，例如:2013-10-25  （n = 5）
	 * 执行后返回 2013-10-30
	 * @param n
	 * 					当前日期加N天
	 * @return Date
