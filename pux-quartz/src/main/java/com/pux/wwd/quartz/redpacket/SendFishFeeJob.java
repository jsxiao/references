 ...
 *
 */
public class SendFishFeeJob implements Job{

	private static Logger logger = LoggerFactory.getLogger(SendFishFeeJob.class);
	...
	public void execute(JobExecutionContext context)
		...
		if (context != null) {
			...
			try {
				System.out.println("执行新手标添加利息发放任务 开始 " + str + new MyDate() + " by " + jobKey);
				logger.info("执行新手标添加利息发放任务 开始: {} by: {} ", str + new MyDate(), jobKey);
				
				IAutoRunService autoRunService = ManagerMain.getCtx().getBean(IAutoRunService.class);
				autoRunService.sendFishFee();
				
				System.out.println("执行新手标添加利息发放任务 结束 " + str + new MyDate() + " by " + jobKey);
				logger.info("执行新手标添加利息发放任务 结束 : {} by: {}", str + new MyDate(), jobKey);
			} catch (Exception e) {
